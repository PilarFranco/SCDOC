...
if ($id_empresa == 1) { //admin
  $sel = $con->query("SELECT ge.id, ge.title, ge.description, ge.url, emp.emp_nombre from general ge
  inner JOIN empresas emp on ge.id_empresa= emp.id_empresa");
  ...
  $sel = $con->query("SELECT * FROM general WHERE id_empresa = '$id_empresa'");
  while ($row = $sel->fetch_assoc()) {
    $tmp = $row;
    array_push($res, $tmp);
  }
                ...
                <tbody>
                  <?php foreach ($res as $val) { ?>
                    ...
                    <?php if ($id_empresa == 1) { ?>                  
                      ...
                      <td>
                        <a class="btn btn-primary" target="_black" href="<?php echo 'http://' . $_SERVER['HTTP_HOST'] . '/SCDOC/views/General/' . $val['url']; ?>">Ver Archivo pagina</a>
