...
if ($id_empresa == 1) { //admin
  ...
  $sel = $con->query("SELECT ar.id, ar.title, ar.description, ar.url, emp.emp_nombre from arcperso ar
  inner JOIN empresas emp 
 on ar.id_empresa= emp.id_empresa WHERE ar.id_empresa = '$id_empresa'");
  while ($row = $sel->fetch_assoc()) {
    $tmp = $row;
    array_push($res, $tmp);
  }
                ...
                <tbody>
                  <?php foreach ($res as $val) { ?>
                    <tr>
                      ...
                      <td>
                      <a class="btn btn-primary" target="_black" href="<?php echo 'http://' . $_SERVER['HTTP_HOST'] . '/SCDOC/views/CrudPersonal/' . $val['url']; ?>">Ver Archivo pagina</a>
                      </td>
                    </tr>
                  <?php } ?>
                </tbody>
